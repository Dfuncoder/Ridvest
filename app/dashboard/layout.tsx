...
function NavLink({ item, onClick }: { item: typeof navItems[0]; onClick?: () => void }) {
  ...
  return (
    ...
    >
      ...
      {item.badge ? (
        <span className="w-5 h-5 rounded-full bg-amber-400 text-[#0d2137] text-[10px] font-extrabold flex items-center justify-center shrink-0">
          {item.badge}
        </span>
      ) : active ? (
    ...
    <div className="min-h-screen bg-slate-100 flex">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-[#0d2137] shrink-0 fixed top-0 left-0 h-full z-30">
        ...
        <nav className="flex-1 px-3 py-4 flex flex-col gap-0.5 overflow-y-auto">
          ...
          {navItems.slice(0, 5).map((item) => <NavLink key={item.href} item={item} />)}
          <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest px-3 mb-2 mt-5">Account</p>
          {navItems.slice(5).map((item) => <NavLink key={item.href} item={item} />)}
        </nav>

        <div className="px-3 py-4 border-t border-white/8">
          <div className="flex items-center gap-3 px-3 py-3 rounded-xl bg-white/5 mb-1">
            <div className="w-8 h-8 rounded-full bg-amber-400 flex items-center justify-center text-[#0d2137] font-extrabold text-sm shrink-0">J</div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-white truncate">Jude Mbakwe</p>
              <p className="text-xs text-slate-500 truncate">hello@ridvest.ng</p>
            </div>
          </div>
          <Link href="/login" className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm text-slate-500 hover:text-red-400 hover:bg-red-400/5 transition-all">
      ...
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <aside className="relative flex flex-col w-72 bg-[#0d2137] h-full z-50">
            ...
            <nav className="flex-1 px-3 py-4 flex flex-col gap-0.5 overflow-y-auto">
              <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest px-3 mb-2">Main</p>
              {navItems.slice(0, 5).map((item) => <NavLink key={item.href} item={item} onClick={() => setSidebarOpen(false)} />)}
              <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest px-3 mb-2 mt-5">Account</p>
              {navItems.slice(5).map((item) => <NavLink key={item.href} item={item} onClick={() => setSidebarOpen(false)} />)}
            </nav>
            <div className="px-3 py-4 border-t border-white/8">
              <div className="flex items-center gap-3 px-3 py-3 rounded-xl bg-white/5 mb-1">
                <div className="w-8 h-8 rounded-full bg-amber-400 flex items-center justify-center text-[#0d2137] font-extrabold text-sm shrink-0">J</div>
      ...
      <div className="flex-1 lg:ml-64 flex flex-col min-h-screen">
        ...
        {/* Topbar */}
        <header className="sticky top-0 z-20 bg-white/90 backdrop-blur-md border-b border-slate-200 px-5 lg:px-8 h-16 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(true)} className="lg:hidden w-9 h-9 flex items-center justify-center rounded-lg text-slate-600 hover:bg-slate-100 transition-colors">
            ...
            <div>
              <p className="text-xs text-slate-400 leading-none mb-0.5">Good morning 👋</p>
              <p className="text-sm font-bold text-slate-900 leading-none">Jude Mbakwe</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Link href="/dashboard/notifications" className="relative w-9 h-9 flex items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100 transition-colors">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0" /></svg>
              <span className="absolute top-2 right-2 w-2 h-2 rounded-full bg-amber-400 border border-white" />
            </Link>
            <Link href="/dashboard/profile" className="w-9 h-9 rounded-full bg-[#0d2137] flex items-center justify-center text-amber-400 font-extrabold text-sm">J</Link>
          </div>
        </header>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-30 bg-white border-t border-slate-200 grid grid-cols-5">
        {navItems.slice(0, 5).map((item) => {
          ...
          const active = item.exact ? pathname === item.href : pathname.startsWith(item.href);
          return (
            <Link key={item.href} href={item.href} className={`flex flex-col items-center justify-center py-2.5 gap-0.5 text-[10px] font-medium transition-colors ${active ? "text-amber-500" : "text-slate-400"}`}>
              <div className="relative">
                {item.icon}
                {item.badge ? <span className="absolute -top-1 -right-1.5 w-4 h-4 rounded-full bg-amber-400 text-[#0d2137] text-[9px] font-extrabold flex items-center justify-center">{item.badge}</span> : null}
              </div>
              {item.label}
            </Link>
          );
